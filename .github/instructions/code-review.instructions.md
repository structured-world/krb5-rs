...
# Code Review Instructions for krb5-rs

## Scope Rules (CRITICAL)

- **Review ONLY code within the PR's diff.** Do not suggest inline fixes for unchanged lines.
- For issues in code **outside the diff**, suggest creating a **separate issue** instead of proposing code changes. Example: "Consider opening an issue to add ASN.1 length validation here -- this is outside this PR's scope."
...
- Corruption/validation tests: tamper the relevant field (e.g., ciphertext byte, ASN.1 tag, checksum) and assert the error
- Use same serialization as production (e.g., DER encoding, not shortcuts)
